...
"""`predictably`'s core panel data type.

Users should not instantiate the `Panel` class directly. Use the `from_array`
...
__all__: list[str] = ["Panel"]


@attrs.define(kw_only=True, slots=False)
class Panel(BasePredictablyDataType):  # numpydoc ignore=PR02
    """`predictably` panel data type.

    Provides core functionality for declaring and interacting with a dataset a
    panel dataset where cross-sectional observations (instances) are observed
    over time.
